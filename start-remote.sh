...
# ═══════════════════════════════════════════════════════════════
# NOFX AI Trading System - Remote Image Start Script
# Usage: ./start-remote.sh [command]
# ═══════════════════════════════════════════════════════════════
...
# 默认使用远程镜像版 Compose 文件，保持与上游 docker-compose.yml 解耦
COMPOSE_FILE_REMOTE="docker-compose.remote.yml"
COMPOSE_STACK_CMD=""
...
# ------------------------------------------------------------------------
# Validation: Remote Compose File
# ------------------------------------------------------------------------
ensure_remote_compose() {
    if [ ! -f "$COMPOSE_FILE_REMOTE" ]; then
        print_error "未找到 $COMPOSE_FILE_REMOTE"
        print_info "请确认已从 docker-compose.yml 复制并修改远程镜像版"
        exit 1
    fi
    print_success "Compose 文件存在: $COMPOSE_FILE_REMOTE"
}
...
check_docker() {
    ...
    detect_compose_cmd
    ensure_remote_compose
    COMPOSE_STACK_CMD="$COMPOSE_CMD -f $COMPOSE_FILE_REMOTE"
    print_info "后续命令将使用: $COMPOSE_STACK_CMD"
...
start() {
    ...
    if [ "$1" == "--build" ]; then
        print_info "重新构建本地镜像..."
        $COMPOSE_STACK_CMD up -d --build
    else
        print_info "拉取远程镜像..."
        $COMPOSE_STACK_CMD pull
        print_info "启动容器..."
        $COMPOSE_STACK_CMD up -d
    fi
    ...
    print_info ""
    print_info "查看日志: ./${SCRIPT_NAME} logs"
    print_info "停止服务: ./${SCRIPT_NAME} stop"
}
...
logs() {
    if [ -z "$2" ]; then
        $COMPOSE_STACK_CMD logs -f
    else
        $COMPOSE_STACK_CMD logs -f "$2"
    fi
}
...
# ------------------------------------------------------------------------
show_help() {
    echo "NOFX AI Trading System - 远程镜像 Docker 管理脚本"
    echo ""
    ...
    echo ""
    echo "示例:"
    echo "  ./${SCRIPT_NAME} start            # 拉取镜像并启动"
    echo "  ./${SCRIPT_NAME} start --build    # 本地重新构建"
    echo "  ./${SCRIPT_NAME} logs backend     # 查看后端日志"
    echo "  ./${SCRIPT_NAME} status           # 查看状态"
    echo "  ./${SCRIPT_NAME} setup-encryption # 手动设置加密环境"
    echo ""
    echo "🔐 关于加密:"
